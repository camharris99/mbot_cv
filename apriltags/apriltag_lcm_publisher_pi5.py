...
Adapted to work with Raspberry Pi 5
"""
...
class Camera:
    def __init__(self, camera_id, width, height):
        ...
        config["transform"] = libcamera.Transform(hflip=1, vflip=1) # Change as needed
        self.cap.align_configuration(config)
        self.cap.configure(config)
        self.cap.start()        
        
        self.detector = Detector(searchpath=['~/apriltag'],
    ...
    def publish_apriltag(self):
        ...
        msg.utime = int(time.time_ns()/1000)
...
if __name__ == '__main__':
    # image width and height here should align with save_image.py (calibration data!)
    camera_id = 0
    image_width = 1640
    image_height = 922
